...
# mask_gom[(lon_msh<=-89.75) & (lat_msh>=29.5)] = np.nan
#%% Figure 1
lat_bouy = [25.942,26.055,25.925,27.91,26.968,29.232,28.501,28.787,29.207,24.456,28.905,24.693,29.408]
lon_bouy = [-89.657,-93.646,-85.615,-95.345,-96.693,-94.413,-84.508,-86.007,-88.237,-81.877,-89.428,-82.773,-84.858]
...
for i, ax in enumerate(axgr):
    ...
    ax.set_ylim([18,31])
    if i<1:
        p1 = ax.contourf(lon_socat, lat_socat, depth*mask['gom'], transform=projection,shading='interp',cmap='Blues_r',levels=np.arange(-4000,1,500),extend='min')
        ax.scatter(lon_bouy,lat_bouy,30,c='tab:pink')
        # cs = ax.contour(lon_socat, lat_socat, depth*mask['gom'], transform=projection,shading='interp',cmap='Blues_r',levels=np.arange(-4000,1,500),linewidth=1.5,color='k')
        # ax.clabel(cs,inline=True,fontsize=10)
        ax.text(-96,28,'TXS',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-92,28.3,'LAS',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-84,26,'WFS',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-96.5,24.5,'nOPEN-W',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-92,24.5,'nOPEN-M',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-86.1,24.2,'nOPEN-E',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-96,22,'CB',fontdict={'size':10,'weight':'bold','color':'red'})
        ax.text(-89,22,'YS',fontdict={'size':10,'weight':'bold','color':'red'})

        ax.text(-99,31.5,'(a) Gulf of Mexico topography',fontdict={'size':12,'weight':'bold'})
    else:
        p2 = ax.scatter(df_socat_qc['longitude'].values, df_socat_qc['latitude'].values, 1,df_socat_qc['pco2sw'].values, transform=projection,cmap=plt.get_cmap('RdYlBu_r',14),clim=[320,460])
        ax.text(-99,31.5,'(b) SOCAT measured pCO$_{2}$',fontdict={'size':12,'weight':'bold'})
    # cs = ax.pcolor(lon_socat, lat_socat, depth*mask['gom'], transform=projection,shading='interp',clim=[-3500,0],cmap='Blues_r')
    # titlefig = '('+chr(97+i)+') ' + calendar.month_name[i+1]
    ...
    ax.contour(lon_socat,lat_socat,depth*mask['ngom'],levels=[-200],colors='k',linewidth=1,linestyles='-')

cb1 = axgr.cbar_axes[0].colorbar(p1,extend='min',label='Depth (m)')
cb2 = axgr.cbar_axes[1].colorbar(p2,extend='both',label='$\it{p}CO_{2}$ (µatm)')
plt.tight_layout()
plt.savefig('figs/Fig01_Topo_SOCAT.jpg',dpi=600,)
plt.show()
#%% Figure 1
...
#     ax.set_ylim([18,31])
#     if i<1:
#         p1 = ax.contourf(lon_socat, lat_socat, depth*mask['gom'], transform=projection,shading='interp',cmap='Blues_r',levels=np.arange(-4000,1,500))
#         # cs = ax.contour(lon_socat, lat_socat, depth*mask['gom'], transform=projection,shading='interp',cmap='Blues_r',levels=np.arange(-4000,1,500),linewidth=1.5,color='k')
#         # ax.clabel(cs,inline=True,fontsize=10)
#         ax.text(-96,28,'TXS',fontdict={'size':10,'weight':'bold','color':'red'})
#         ax.text(-92,28.3,'LAS',fontdict={'size':10,'weight':'bold','color':'red'})
...
#         ax.text(-99,31.5,'(a) Gulf of Mexico topography',fontdict={'size':12,'weight':'bold'})
#     elif i<2:
#         p2 = ax.scatter(df_socat_qc['longitude'].values, df_socat_qc['latitude'].values, 1,df_socat_qc['fCO2rec'].values, transform=projection,cmap='RdYlBu_r',clim=[320,460])
#         ax.text(-99,31.5,'(b) Discrete measured fCO2 (all samples)',fontdict={'size':12,'weight':'bold'})
#     # cs = ax.pcolor(lon_socat, lat_socat, depth*mask['gom'], transform=projection,shading='interp',clim=[-3500,0],cmap='Blues_r')
#     # titlefig = '('+chr(97+i)+') ' + calendar.month_name[i+1]
#     else:
#         p3 = ax.pcolor(lon_socat, lat_socat, fco2sw_seas_clim[i-2]*mask['gom'], transform=projection, cmap='RdBu_r', clim=[320,460],shading='interp')
#         titlefig = '('+chr(97+i)+') Gridded fCO$_{2sw}$, '+str_seas[i-2]
#         ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
...
# cb1 = axgr.cbar_axes[0].colorbar(p1,extend='min',label='Depth (m)')
# cb2 = axgr.cbar_axes[1].colorbar(p2,extend='both',label='fCO$_2$ (µatm)')
# for i in range(2,6):
#     axgr.cbar_axes[i].colorbar(p2,extend='both',label='fCO$_2$ (µatm)')
# plt.tight_layout()
# plt.savefig('figs/Fig01_Topo_SOCAT2.jpg',dpi=600)
# plt.show()
#%% Figure 2
...
for i, ax in enumerate(axgr):
    ...
    p3 = ax.pcolor(lon_socat, lat_socat, fco2sw_seas_clim[i]*mask['gom'], transform=projection, cmap=plt.get_cmap('RdYlBu_r',14), clim=[320,460],shading='interp')
    # p3 = ax.contourf(lon_socat, lat_socat, fco2sw_seas_clim[i]*mask['gom'], transform=projection, cmap=plt.get_cmap('RdYlBu_r',14), clim=[320,460],extend='both')
    titlefig = '('+chr(97+i)+') Gridded pCO$_2$, '+str_seas[i]
    ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
...
cb = axgr.cbar_axes[0].colorbar(p3,extend='both',label='$\it{p}CO_2$ (µatm)')
plt.tight_layout()
plt.savefig('figs/Fig02_fCO2seasonalMap.jpg',dpi=600)
plt.show()
#%% Figure Ax
...
for i, ax in enumerate(axgr):
    ...
    ax.set_ylim([18,31])
    if i <4:
        p1 = ax.pcolor(lon_socat, lat_socat, sst_seas_clim[i]*mask['gom'], transform=projection, cmap='RdBu_r', clim=[15,35],shading='interp')
        titlefig = '('+chr(97+i)+') SST, '+str_seas[i]
        ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
        ax.plot([-97.8,-82,],[b_nsgom,b_nsgom],'k',linewidth=1)
        ax.plot([b_txs_las,b_txs_las,],[b_nsgom,29.8],'k',linewidth=1)
        ax.plot([b_las_wfs,b_las_wfs,],[b_nsgom,30.1],'k',linewidth=1)
        ...
        p2 = ax.pcolor(lon_socat, lat_socat, sss_seas_clim[i-4]*mask['gom'], transform=projection, cmap='RdBu_r', clim=[27,40],shading='interp')
        titlefig = '('+chr(97+i)+') SSS, '+str_seas[i-4]
        ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
        ...
        p3 = ax.pcolor(lon_socat, lat_socat, wspd_seas_clim[i-8]*mask['gom'], transform=projection, cmap='RdYlBu_r', clim=[3,8],shading='interp')
        # p3 = ax.contourf(lon_socat, lat_socat, wspd_seas_clim[i-8]*mask['gom'], transform=projection, cmap='RdYlBu_r', shading='interp',levels=np.arange(3,8.1,0.5))
        cs = ax.contour(lon_socat, lat_socat, wspd_seas_clim[i-8]*mask['gom'], transform=projection,alpha=0.5,colors='grey',linewidth=0.5,levels=np.arange(3,8.1,0.5))
        ax.clabel(cs,inline=True,fontsize=8)
        step = 6
        ax.quiver(lon_socat[::step],lat_socat[::step],uwnd_seas_clim[i-8,::step,::step]*mask['gom'][::step,::step],vwnd_seas_clim[i-8,::step,::step]*mask['gom'][::step,::step],  transform=projection)
        titlefig = '('+chr(97+i)+') U$_{10}$, '+str_seas[i-8]
        ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
        ...
        p4 = ax.pcolor(lon_socat, lat_socat, dfco2_seas_clim[i-12]*mask['gom'], transform=projection, cmap='RdBu_r', clim=[-100,100],shading='interp')
        titlefig = '('+chr(97+i)+') ΔfCO$_2$, '+str_seas[i-12]
        ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
...
cb1 = axgr.cbar_axes[0].colorbar(p1,extend='both',label='SST (℃)')
axgr.cbar_axes[1].colorbar(p2,extend='both',label='SSS')
axgr.cbar_axes[2].colorbar(p3,extend='both',label='U$_{10}$ (m/s)')
axgr.cbar_axes[3].colorbar(p4,extend='both',label='Δ$\it{p}CO_2$ (µatm)')
plt.tight_layout()
plt.savefig('figs/FigA_seasonal_SST_SSS_map.jpg',dpi=600)
plt.show()
#%%
# fig = plt.figure(figsize=(16,12))
# axgr = AxesGrid(fig, 111, axes_class=axes_class,
...
#         ax.contour(lon_socat,lat_socat,depth*mask['ngom'],levels=[-200],colors='k',linewidth=1,linestyles='-')
#     elif i <8:
#         p2 = ax.pcolor(lon_socat, lat_socat, sss_seas_clim[i-4]*mask['gom'], transform=projection, cmap='RdBu_r', clim=[29,38],shading='interp')
#         titlefig = '('+chr(97+i)+') SSS, '+str_seas[i-4]
#         ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
...
#     else:
#         p4 = ax.pcolor(lon_socat, lat_socat, fco2sw_seas_clim[i-12]*mask['gom'], transform=projection, cmap='RdBu_r', clim=[320,460],shading='interp')
#         titlefig = '('+chr(97+i)+') fCO$_{2sw}$, '+str_seas[i-12]
#         ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
...
# axgr.cbar_axes[1].colorbar(p2,extend='both',label='SSS')
# axgr.cbar_axes[2].colorbar(p3,extend='both',label='U$_{10}$ (m/s)')
# axgr.cbar_axes[3].colorbar(p4,extend='both',label='fCO$_{2sw}$ (µatm)')
# plt.tight_layout()
# plt.savefig('figs/Figure_seasonal_map2.jpg',dpi=600)
# plt.show()
# %%
flux_clim_map_monthly = np.nanmean(np.reshape(flux,(int(flux.shape[0]/12),12,flux.shape[1],flux.shape[2])),axis=0)
import calendar
...
for i, ax in enumerate(axgr):
    ...
    ax.set_xlim([-98.5,-80])
    ax.set_ylim([18,31])
    p = ax.pcolor(lon_socat, lat_socat, flux_clim_map_monthly[i]*mask['gom'], transform=projection, cmap=plt.get_cmap('RdYlBu_r',16), clim=[-2,2],shading='interp')
    titlefig = '('+chr(97+i)+') ' + calendar.month_name[i+1]
    ax.text(-99,31.5,titlefig,fontdict={'size':12,'weight':'bold'})
...
cb = axgr.cbar_axes[0].colorbar(p,extend='both',label='CO$_2$ Flux (mol C/m$^2$/yr)')
plt.tight_layout()
plt.savefig('figs/Figure_Flux_map.jpg',dpi=600)
plt.show()

# %% Figure 03
ts_df_dict = datapreprocessing(df_fco2sw,False)
ts_stats1, var_trends1, wls_model_dict1, TDTi_dict1 = calc_all_stats(ts_df_dict, var_unc_dict_ana, var_sigfig_dict_ana)
ts_df_dict = datapreprocessing(df_fco2air,False)
ts_stats2, var_trends2, wls_model_dict2, TDTi_dict2 = calc_all_stats(ts_df_dict, var_unc_dict_ana, var_sigfig_dict_ana)
ts_df_dict = datapreprocessing(df_dfco2,False)
ts_stats3, var_trends3, wls_model_dict3, TDTi_dict3 = calc_all_stats(ts_df_dict, var_unc_dict_ana, var_sigfig_dict_ana)

keys = ['txs','las','wfs','nopenw','nopenm','nopene','cb','ys']
regionnames = ['TXS','LAS','WFS','nOPEN-W','nOPEN-M','nOPEN-E','CB','YS']
...
plt.subplots_adjust(left=0.05,
                    ...
                    wspace=0.25,
                    hspace=0.3)
for idxkey in range(9):
    if idxkey<8:
        key = keys[idxkey]
        plt.subplot(3,3,idxkey+1)
        x = range(1,13)
        y1 = ts_stats1[key]['monthly'][key]['mean']
        yerr1 = ts_stats1[key]['monthly'][key]['std']
        y2 = ts_stats2[key]['monthly'][key]['mean']
        yerr2 = ts_stats2[key]['monthly'][key]['std']
        p1 = plt.errorbar(x,y1,yerr=yerr1,fmt='o-', markersize=5, capsize=3,label='fCO$_{2sw}$')
        p2 = plt.errorbar(x,y2,yerr=yerr2,fmt='--', markersize=3, capsize=2,label='fCO$_{2air}$')

        y1_avg = np.nanmean(y1)
        y1_std = np.nanstd(ts_stats1[key]['annual'][key]['mean'].values)
        y3 = ts_stats3[key]['monthly'][key]['mean'].values
        y3_avg = np.nanmean(y3)
        y3_std = np.nanstd(ts_stats3[key]['annual'][key]['mean'].values)

        str_y1avg = 'annual avg. $\it{p}CO_{2sw}$ = '+ str(np.round(y1_avg,1)) + ' ± ' + str(np.round(y1_std,1)) + ' µatm'
        if y3_avg>0:
            str_y3avg = 'annual avg. Δ$\it{p}CO_{2sw}$ = +'+ str(np.round(y3_avg,1)) + ' ± ' + str(np.round(y3_std,1)) + ' µatm'
        else:
            str_y3avg = 'annual avg. Δ$\it{p}CO_{2sw}$ = '+ str(np.round(y3_avg,1)) + ' ± ' + str(np.round(y3_std,1)) + ' µatm'

        plt.text(1.5,480,str_y1avg,fontdict={'size':10})
        plt.text(1.5,270,str_y3avg,fontdict={'size':10})

        plt.grid('on',alpha=0.5)
        plt.ylim(np.array([240,510]))
        plt.yticks(np.arange(250,501,50))
        plt.xticks(range(1,13))
        plt.ylabel('$\it{p}CO_{2}$ (µatm)',fontdict={'size':9})
        plt.text(0,520,'('+chr(97+idxkey)+') ' + regionnames[idxkey],fontdict={'size':11,'weight':'bold'})
    else:
        plt.subplot(3,3,idxkey+1)
        p1 = plt.errorbar(np.nan,np.nan,yerr=np.nan,fmt='o-', markersize=5, capsize=3,label='$\it{p}CO_{2sw}$',)
        p2 = plt.errorbar(np.nan,np.nan,yerr=np.nan,fmt='--', markersize=3, capsize=2,label='$\it{p}CO_{2air}$')
        plt.axis('off')
        plt.legend(loc='center left',ncols=2)
plt.savefig('figs/Fig03_Regional_pCO2sw.jpg',dpi=600)
# %% Figure 04
ts_df_dict = datapreprocessing(df_flux,False)
ts_stats, var_trends, wls_model_dict, TDTi_dict = calc_all_stats(ts_df_dict, var_unc_dict_ana, var_sigfig_dict_ana)
keys = ['txs','las','wfs','nopenw','nopenm','nopene','cb','ys']
...
plt.subplots_adjust(left=0.05,
                    ...
                    wspace=0.25,
                    hspace=0.3)
for idxkey in range(len(keys)):
    key = keys[idxkey]
    plt.subplot(3,3,idxkey+1)
    x = np.arange(1,13)
    y1 = ts_stats[key]['monthly'][key]['mean'].values
    yerr1 = ts_stats[key]['monthly'][key]['std'].values
    y1_avg = np.nanmean(y1)
    y1_std = np.nanstd(ts_stats[key]['annual'][key]['mean'].values)
    if y1_avg>0:
        str_y1avg = 'annual avg. F$_{CO2}$ = +'+ "{:.2f}".format(y1_avg) + ' ± ' + "{:.2f}".format(y1_std) + ' mol C/m$^2$/yr'
    else:
        str_y1avg = 'annual avg. F$_{CO2}$ = '+ "{:.2f}".format(y1_avg) + ' ± ' + "{:.2f}".format(y1_std) + ' mol C/m$^2$/yr'
    
    plt.errorbar(x,y1,yerr=yerr1,fmt='o-', markersize=5, capsize=3,)
    idxp = np.where(y1>0)[0]
    plt.errorbar(x[idxp],y1[idxp],yerr=yerr1[idxp],fmt='o', markersize=5, capsize=3,color='r')
    plt.grid('on',alpha=0.3)
    plt.ylim(np.array([-4.5,2.5]))
    plt.xlim(np.array([0.5,12.5]))
    plt.xticks(range(1,13))
    plt.yticks(np.arange(-4,2.1,1))
    plt.ylabel('CO$_2$ Flux (mol C/m$^2$/yr)',fontdict={'size':9})
    plt.text(0,2.7,'('+chr(97+idxkey)+') ' + regionnames[idxkey],fontdict={'size':11,'weight':'bold'})
    plt.text(1.5,-3.5,str_y1avg,fontdict={'size':10})
plt.savefig('figs/Fig04_Regional_Flux.jpg',dpi=600)
# %%
keys = ['txs','las','wfs','nopenw','nopenm','nopene','cb','ys']
regionnames = ['TXS','LAS','WFS','nOPEN-W','nOPEN-M','nOPEN-E','CB','YS']
...
for idxkey in range(9):
    if idxkey<8:
        key = keys[idxkey]
        ax1 = plt.subplot(3,3,idxkey+1)

        ts_df_dict = datapreprocessing(df_flux[['time',key]],True)
        # ts_df_dict = datapreprocessing(df_flux,False)
        ts_stats, var_trends, wls_model_dict, TDTi_dict = calc_all_stats(ts_df_dict, var_unc_dict_ana, var_sigfig_dict_ana)

        # x1 = wls_model_dict[key]['decimal_year_zero'][:,1]+yr_min
        x1 = pd.to_datetime(ts_df_dict[key].index)
        y1 = wls_model_dict[key]['ts_variable_deseasoned'] - np.nanmean(ts_stats[key]['monthly'][key]['mean'].values)
        y1fit = wls_model_dict[key]['trend'] - np.nanmean(ts_stats[key]['monthly'][key]['mean'].values)
        tr_str1 = 'Flux trend = ' + "{:+.3f}".format(np.float32(wls_model_dict[key]['slope_str'])) + ' ± ' + "{:.3f}".format(np.float32(wls_model_dict[key]['slope_err_str'])) + ' mol C/m$^2$/yr$^2$'

        ts_df_dict = datapreprocessing(df_fco2sw[['time',key]],True)
        # ts_df_dict = datapreprocessing(df_flux,False)
        ts_stats, var_trends, wls_model_dict, TDTi_dict = calc_all_stats(ts_df_dict, var_unc_dict_ana, var_sigfig_dict_ana)
        # x2 = wls_model_dict[key]['decimal_year_zero'][:,1]+yr_min
        x2 = pd.to_datetime(ts_df_dict[key].index)
        y2 = wls_model_dict[key]['ts_variable_deseasoned'] - np.nanmean(ts_stats[key]['monthly'][key]['mean'].values)
        y2fit = wls_model_dict[key]['trend'] - np.nanmean(ts_stats[key]['monthly'][key]['mean'].values)
        tr_str2 = '$\it{p}CO_{2sw}$ trend = ' + "{:+.2f}".format(np.float32(wls_model_dict[key]['slope_str'])) + ' ± ' + "{:.2f}".format(np.float32(wls_model_dict[key]['slope_err_str'])) + ' µatm/yr'

        tdt_str = '$\it{p}CO_{2sw}$ TDT = ' + "{:.0f}".format(TDTi_dict[key]['TDTi'][0]) + ' ± ' + TDTi_dict[key]['uncert_TDT'][:2] + ' yrs'

        color1 = 'tab:red'
        # ax1.set_xlabel('time')
        ax1.set_ylabel('F$_{CO2}$ Anom. (mol C/m$^2$/yr)', color=color1)  
        ax1.scatter(x1, y1, color='none',edgecolors=color1,marker='^',s=15)
        ax1.plot(x1,y1fit,color=color1,linewidth=2)
        ...
        color2 = 'tab:blue'
        ax2.set_ylabel('$\it{p}CO_{2sw}$ Anom. (µatm)', color=color2)
        ax2.scatter(x2, y2,color='none',edgecolors=color2,marker='o',s=15)
        ax2.tick_params(axis='y', labelcolor=color2)
        ax2.plot(x2,y2fit,color=color2,linewidth=2)
        # ax2.set_ylim([200,600])
        ax2.set_ylim([-150,150])
        ax2.set_xlim([time_min,time_max])
        ax2.set_xticks(time[0::36])
        ax2.set_xticklabels(time[0::36].year)

        ax1.text(time_min+180,2.5,tr_str1,color=color1,fontdict={'size':9})
        ax1.text(time_min+180,-2.0,tdt_str,color=color2,fontdict={'size':9})
        ax1.text(time_min+180,-2.5,tr_str2,color=color2,fontdict={'size':9})
        ax1.text(time_min-180,3.15,'('+chr(97+idxkey)+') ' + regionnames[idxkey],fontdict={'size':11,'weight':'bold'})
    else:
        plt.subplot(3,3,idxkey+1)
        s1 = plt.scatter(np.nan, np.nan,color='none',edgecolors=color1,marker='^',s=15,label='F$_{CO2}$ monthly anomalies')
        s2 = plt.scatter(np.nan, np.nan, color='none',edgecolors=color2,marker='o',s=15,label='fCO$_{2}$ monthly anomalies')
        p1 = plt.plot(np.nan, np.nan,color=color1,linewidth=2,label='Fitted F$_{CO2}$ monthly anomalies')
        p2 = plt.plot(np.nan, np.nan,color=color2,linewidth=2,label='Fitted fCO$_{2sw}$ monthly anomalies')
        plt.axis('off')
        plt.legend(loc='center left',ncols=1)
plt.tight_layout()  # otherwise the right y-label is slightly clipped
plt.savefig('figs/Fig05_Regional_Trend.jpg',dpi=600)
plt.show()
# %%
ts_fco2 = dict({})
ts_dfco2 = dict({})
...
for i in range(3):
    ...
    plt.text(-70,-80-35,'R$^2$ = ' + '{:.2f}'.format(lm_ther_warm.rsquared) +', p = '+'{:.2f}'.format(lm_ther_warm.pvalues[1]) + ' (summer)',color='tab:red')
    plt.text(-70,-95-35,'R$^2$ = ' + '{:.2f}'.format(lm_ther_cold.rsquared) +', p = '+'{:.2f}'.format(lm_ther_cold.pvalues[1]) + ' (winter)',color='tab:blue')
    plt.text(-70,-110-35,'R$^2$ = ' + '{:.2f}'.format(lm_ther_all.rsquared) +', p = '+'{:.2f}'.format(lm_ther_all.pvalues[1]) + ' (all)')
    plt.text(-150,155,'('+chr(97+i)+') '+rnames[i] +' thermal vs. total',fontdict={'size':13,'weight':'bold'})
    ...
    plt.text(-145,140-10,'R$^2$ = ' + '{:.2f}'.format(lm_nonther_warm.rsquared) +', p = '+'{:.2f}'.format(lm_nonther_warm.pvalues[1]) + ' (summer)',color='tab:red')
    plt.text(-145,125-10,'R$^2$ = ' + '{:.2f}'.format(lm_nonther_cold.rsquared) +', p = '+'{:.2f}'.format(lm_nonther_cold.pvalues[1]) + ' (winter)',color='tab:blue')
    plt.text(-145,110-10,'R$^2$ = ' + '{:.2f}'.format(lm_nonther_all.rsquared) +', p = '+'{:.2f}'.format(lm_nonther_all.pvalues[1]) + ' (all)')
    plt.text(-150,155,'('+chr(97+3+i)+') '+rnames[i] +' nonthermal vs. total',fontdict={'size':13,'weight':'bold'})
# plt.tight_layout()
plt.savefig('figs/Fig06_Coastal_TherNonTher.jpg',dpi=600)
# %%
...
# plt.tight_layout()
plt.savefig('figs/Fig07_Coastal_TherNonTher.jpg',dpi=600)
# %%
keys = ['cb','ys']
rnames = ['CB','YS']
...
for i in range(2):
    ...
    if i>0:
        plt.text(-50,-80-10,'R$^2$ = ' + '{:.2f}'.format(lm_ther_warm.rsquared) +', p = '+'{:.2f}'.format(lm_ther_warm.pvalues[1]) + ' (warm)',color='tab:red')
        plt.text(-50,-95-10,'R$^2$ = ' + '{:.2f}'.format(lm_ther_cold.rsquared) +', p = '+'{:.2f}'.format(lm_ther_cold.pvalues[1]) + ' (cold)',color='tab:blue')
        plt.text(-50,-110-10,'R$^2$ = ' + '{:.2f}'.format(lm_ther_all.rsquared) +', p = '+'{:.2f}'.format(lm_ther_all.pvalues[1]) + ' (all)')
    plt.text(-130,135,'('+chr(97+i)+') '+rnames[i] +' thermal vs. total',fontdict={'size':13,'weight':'bold'})
    ...
    if i>0:
        plt.text(-50,-80-10,'R$^2$ = ' + '{:.2f}'.format(lm_nonther_warm.rsquared) +', p = '+'{:.2f}'.format(lm_nonther_warm.pvalues[1]) + ' (warm)',color='tab:red')
        plt.text(-50,-95-10,'R$^2$ = ' + '{:.2f}'.format(lm_nonther_cold.rsquared) +', p = '+'{:.2f}'.format(lm_nonther_cold.pvalues[1]) + ' (cold)',color='tab:blue')
        plt.text(-50,-110-10,'R$^2$ = ' + '{:.2f}'.format(lm_nonther_all.rsquared) +', p = '+'{:.2f}'.format(lm_nonther_all.pvalues[1]) + ' (all)')
    plt.text(-130,135,'('+chr(97+2+i)+') '+rnames[i] +' nonthermal vs. total',fontdict={'size':13,'weight':'bold'})
plt.savefig('figs/FigA_sGOM.jpg',dpi=600)
# %%