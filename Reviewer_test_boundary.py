...
flux_list_ngom = np.array([])
flux_list_gom = np.array([])
for b_txs_las in np.arange(-94,-91.9,0.5):
    for b_las_wfs in np.arange(-87,-84.9,0.5):
        ...
        flux_list_gom = np.append(flux_list_gom,flux_clim_annual['gom'])
        flux_list_ngom = np.append(flux_list_ngom,flux_clim_annual['ngom'])

# print('In the GOM, different boundary leads to flux of %.2f ± %.2f (%.2f)' % (np.nanmean(flux_list_gom),np.nanstd(flux_list_gom),np.nanstd(flux_list_gom)/np.nanmean(flux_list_gom)*100))
print('In the nGOM, different boundary leads to flux of %.2f ± %.2f (%.2f %%)' % (np.nanmean(flux_list_ngom),np.nanstd(flux_list_ngom),np.abs(np.nanstd(flux_list_ngom)/np.nanmean(flux_list_ngom))*100))
# %%
